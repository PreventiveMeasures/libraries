...
// `fn` pass straight through to the formatter, so the label lines and the
// -p names are as much the caller's here as they are there.
//
...
export function diff(a, b, { format = 'unified', context = 3, header = '', fn = null, ...compare } = {}) {
  ...
  return FORMAT[format](from, to, blocks, { context, header, fn })
}
