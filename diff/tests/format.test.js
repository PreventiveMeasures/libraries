...
// that a format recorded twice will not read back the same way twice.
...
// format, and every format has to read back the same: a disagreement means the
...
describe('the corpus is a corpus', () => {
  it('holds a fair number of cases in every format', () => {
    const counted = (format) => RECORDINGS.filter((recording) => recording.format === format).length
    for (const format of ['normal', 'unified', 'context']) assert.ok(counted(format) >= 10, `${format}: ${counted(format)} cases`)
  ...
  it('reads the same change set out of every format that records a pair', () => {
