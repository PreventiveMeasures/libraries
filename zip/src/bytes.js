...
async function through(bytes, stream, limit, at) {
  ...
  for (;;) {
    ...
    if (total > limit) {
      await reader.cancel()
      throw new ZipError('an entry inflates to more than its declared size', at)
    }
    chunks.push(value)
  }
  await writing
...
export async function inflate(bytes, size, at) {
  ...
  try {
    ...
    throw new ZipError('an entry does not inflate', at)
  }
  if (out.length !== size) throw new ZipError('an entry inflates to less than its declared size', at)
  return out
}
...
export function toDos(mtime) {
  ...
  if (year < 1980 || year > 2107) throw new ZipError(`mtime ${mtime} is outside DOS time, 1980 to 2107`)
  return {
...
export function fromDos(date, time, at) {
  ...
  if (same.some((value, i) => value !== fields[i])) throw new ZipError('an entry has an invalid DOS time', at)
  return stamp / 1000
}
