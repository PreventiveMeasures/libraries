...
function integer(value, what, min, max) {
  if (!Number.isSafeInteger(value) || value < min || value > max) throw new ZipError(`${what} ${String(value)} is not an integer from ${min} to ${max}`)
  return value
}

function normalize(entry) {
  if (entry === null || typeof entry !== 'object') throw new ZipError('an entry is not an object')
  const type = entry.type ?? 'file'
  if (!Object.hasOwn(TYPE_BITS, type)) throw new ZipError(`entry type ${quote(String(type))} is not one this package writes`)
  const { name } = entry
  if (typeof name !== 'string') throw new ZipError('entry name is not a string')
  const data = entry.data ?? EMPTY
  if (!(data instanceof Uint8Array)) throw new ZipError(`data of ${quote(name)} is not a Uint8Array`)
  if (data.length !== 0 && type !== 'file') throw new ZipError(`a ${type} cannot carry data (${quote(name)})`)
  const linkname = entry.linkname ?? ''
  if (typeof linkname !== 'string') throw new ZipError(`link target of ${quote(name)} is not a string`)
  if (linkname !== '' && type !== 'symlink') throw new ZipError(`a ${type} cannot have a link target (${quote(name)})`)
  return {
    name,
    type,
    data,
...
export async function zip(entries, { method = 'deflate' } = {}) {
  if (!METHODS.has(method)) throw new ZipError(`method ${quote(String(method))} is not deflate or store`)
  ...
  for (const given of entries) {
    ...
    names.add(e)
    if (++count > 0xffff) throw new ZipError('more than 65535 entries would need zip64')
    ...
    if (body.length > MAX32 || offset + stored.length > MAX32) throw new ZipError(`${quote(e.name)} would need zip64`)
  ...
  if (offset + directory.length > MAX32) throw new ZipError('the archive would need zip64')
