...
import { ZipError, unzip, zip } from '../index.js'
import { concat, deflate, record } from '../src/bytes.js'
...
describe('what it refuses', () => {
  ...
  it('throws ZipError with the offset', async () => {
    // The offset of the second entry's central record: two local entries
    // of 32 bytes, then the first central record of 47.
    await assert.rejects(unzip(archive([x(), entry('a', utf8('y'))])), (error) => error instanceof ZipError && error.offset === 111)
  })
})
