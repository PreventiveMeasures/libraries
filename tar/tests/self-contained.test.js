...
// `tar/` is a package of its own, and the point of it is that it runs
...
// diff.
const TAR_DIR = new URL('../', import.meta.url)
const SRC_DIR = new URL('src/', TAR_DIR)
...
const files = [
  new URL('index.js', TAR_DIR),
  new URL('index.d.ts', TAR_DIR),
...
const manifest = JSON.parse(readFileSync(new URL('package.json', TAR_DIR), 'utf8'))
...
describe('tar/ ships every module it imports', () => {
  ...
  for (const file of files) {
    const name = file.href.slice(TAR_DIR.href.length)
...
describe('tar/ is self-contained and needs no node:', () => {
  it('has files to check', () => {
    assert.ok(files.length > 5, `expected the tar/ modules, found ${files.length}`)
  })
  ...
  for (const file of files) {
    const name = file.href.slice(TAR_DIR.href.length)
    it(`${name} imports nothing outside tar/ and nothing from node:`, () => {
      for (const spec of specifiersOf(readFileSync(file, 'utf8'))) {
        if (!spec.startsWith('.')) {
          const bare = spec.startsWith('@') ? spec.split('/').slice(0, 2).join('/') : spec.split('/')[0]
          assert.ok(!spec.startsWith('node:'), `${name} imports ${spec}: tar/ runs on browser APIs alone`)
          assert.ok(declared.has(bare), `${name} imports ${spec}, which package.json does not declare`)
          continue
        }
        const target = new URL(spec, file)
        assert.ok(target.href.startsWith(TAR_DIR.href), `${name} imports ${spec} — tar/ may only import within itself and declared dependencies`)
      }
    })
  }
