...
describe('symbolic links', () => {
  ...
  it('take a target of any spelling but an empty one', () => {
    ...
    fails(() => fs.hardlink('/x', '/a/'), 'EEXIST', '/a/')
...
describe('hard links', () => {
  it('are one inode under two names', () => {
    ...
    fails(() => fs.hardlink('/b', '/b'), 'EEXIST')
    fails(() => fs.hardlink('/missing', '/c'), 'ENOENT')
    fails(() => fs.hardlink('/', '/c'), 'EPERM')
    fails(() => fs.hardlink('/b', '/c/'), 'ENOENT')
  })
  ...
  it('follow a link spelled with a trailing slash, as lstat does', () => {
    ...
    fails(() => fs.hardlink('/todir/', '/x'), 'EPERM', '/todir/')
    fails(() => fs.hardlink('/tofile/', '/x'), 'ENOTDIR', '/tofile/')
    fails(() => fs.hardlink('/dangling/', '/x'), 'ENOENT', '/dangling/')
    fails(() => fs.hardlink('/loop/', '/x'), 'ELOOP', '/loop/')
  ...
  it('judge the new name before what it would name, as linkat(2) does', () => {
    ...
    fails(() => fs.hardlink('/d', '/f'), 'EEXIST', '/f')
    fails(() => fs.hardlink('/d', '/gone/x'), 'ENOENT', '/gone/x')
    fails(() => fs.hardlink('/d', '/f/x'), 'ENOTDIR', '/f/x')
    fails(() => fs.hardlink('/d', '/x/'), 'ENOENT', '/x/')
    fails(() => fs.hardlink('/d', '/x'), 'EPERM', '/d')
    fails(() => fs.hardlink('/gone', '/f'), 'ENOENT', '/gone')
  })
})
...
describe('rename', () => {
  ...
  it('takes no name from a spelling that ends on . or .., as nothing does', () => {
    ...
    for (const spelling of ['/a/.', '/a/b/..', '/a/./b/.', '/e/.']) {
      ...
      fails(() => fs.hardlink('/a/b/f', spelling), 'EEXIST', spelling)
...
describe('names', () => {
  ...
  it('are text with an encoding, of at most 255 bytes of it', () => {
    ...
    fails(() => fs.hardlink('/f', `/${lone}`), 'EILSEQ')
    ...
    fails(() => fs.hardlink('/f', `/${wide}`), 'ENAMETOOLONG')
