...
describe('entries are the tree as tar would carry it', () => {
  ...
  it('start where they are asked to', () => {
    ...
    assert.deepEqual([...fs.entries('/lib/link')].map((entry) => [entry.name, entry.type]), [['README.md', 'file']], 'a link there is what it leads to, by its own name')
  })
  ...
  it('refuse a name tar would: absolute, .., a backslash, a control or bidirectional character, a drive letter, an empty segment, a slash after what is not a directory', () => {
    ...
    fails(() => vfsFromEntries([{ name: 'a'.repeat(1 << 24), data: 'x' }]), 'ENAMETOOLONG')
    fails(() => vfsFromEntries([{ name: 'ü'.repeat(2049), data: 'x' }]), 'ENAMETOOLONG')
    const wide = `${`${'ü'.repeat(100)}/`.repeat(20)}f`
    assert.equal(vfsFromEntries([{ name: wide, data: 'x' }]).readText(`/${wide}`), 'x', 'bytes, not units, once the units are few enough to count')
  ...
  it('hold a repeat to what the first declaration is held to', () => {
    const replacement = String.fromCodePoint(0xFFFD)
    fails(() => vfsFromEntries([{ name: 'f', data: replacement }, { name: 'f', data: '\uD800' }]), 'EILSEQ', '/f')
    assert.throws(() => vfsFromEntries([{ name: 'f', data: new Uint8Array([1, 2]) }, { name: 'f', data: [1, 2] }]), TypeError)
    assert.throws(() => vfsFromEntries([{ name: 'f' }, { name: 'f', data: { length: 0 } }]), TypeError)
