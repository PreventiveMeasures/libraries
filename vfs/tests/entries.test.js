...
describe('entries are the tree as tar would carry it', () => {
  ...
  it('refuse a name tar would: absolute, .., a backslash, a control or bidirectional character, a drive letter, an empty segment, a slash after what is not a directory', () => {
    ...
    const long = `${'a'.repeat(255)}/`.repeat(15)
    fails(() => vfsFromEntries([{ name: long + 'a'.repeat(242), data: 'x' }]), 'ENAMETOOLONG')
    fails(() => vfsFromEntries([{ name: 'f', data: 'x' }, { name: 'l', type: 'link', linkname: long + 'a'.repeat(242) }]), 'ENAMETOOLONG')
    const longest = vfsFromEntries([{ name: long + 'a'.repeat(241), data: 'x' }])
