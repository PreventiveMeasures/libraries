...
const MODELS = new Map([
  ...
  // List price: Sonnet 5 is discounted to 2 / 10 introductory through 2026-08-31, and hardcoding
  // the intro rate would silently understate every run costed after it expires.
...
// The names the price table knows, in its own order (families together, free models last). Not a
// closed set — `--model` takes any string, and an unknown one simply costs nothing the table can
// price — so this is what to OFFER, never what to allow. The server's console builds its model
// suggestions from it.
...
// Which local build answers a row on Ollama. Separate from the table above because it is Ollama's
