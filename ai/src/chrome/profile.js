...
// The scratch profile a browser is pointed at: where it goes, what it starts
// with, and making sure it is gone again — including when nothing gets to ask
// politely. index.js drives the browser inside it.
...
// than everything the machine has put there. Per user, because the temp dir
// is not always: a shared /tmp takes the mode of whichever account made the
// root, and every other account gets EACCES inside it. Recomputed per call,
// so it follows TMPDIR.
...
export function isScratchProfile(dir) {
  ...
  // Resolved, then read as a name inside a directory. A prefix test on the
  // raw string takes <root>/chrome-x/../../elsewhere for one of ours, and
  // rmSync resolves that traversal before deleting.
...
// Ctrl+C reaches no 'exit' handler on its own: with nothing listening, the
// default disposition ends the process and 'exit' is never emitted. Listening
// suppresses that, so the signal is handed back once the profiles are gone —
// an application listener then decides, and with none the default ends the
// process as it always would. Registered only while a profile of ours exists;
// owning a process's signals past that is not a library's to do.
