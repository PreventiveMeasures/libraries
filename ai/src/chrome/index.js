...
// Testing pins the device performance class to kGpuBlocked, leaving a CPU backend most machines do
// not have.

// Playwright's own list, minus `OptimizationHints`: the on-device model hangs off that keyed
// service, and with it off the model service never starts. A disable cannot be undone by
// --enable-features, so the list is re-sent.
...
// The enable side, the other way round: playwright appends its own AFTER ours and Chrome reads the
// last occurrence, so theirs is dropped and ours carries its entry. ignoreDefaultArgs matches by
// exact string, hence the whole switch.
...
// Which Gemma answers: the Prompt API asks for a USE CASE, and AIApiFoundationalModel:model_version
// keys the manifest's map of them (v4 -> prompt_api_gemma4, v4_4b -> ..._4b, v4_12b -> ..._12b).
// The chrome://flags entry only ever says v4, so its features come from here.
...
// Playwright's software-GL defaults. Both have to go for Chrome to reach a real GPU; see
// launchPersistentContext for why that is not optional.
...
// An optional peer, loaded on demand, so a caller on the hosted providers never pays for a browser
// driver it will not use. The bare module-not-found names neither the package nor the reason it is
// wanted.
...
// One browser per base model: the weights directory is named on the command line, so two rows
// backed by different weights cannot share one.
...
// Order matters only for --use-angle and the two feature lists, which deliberately come after
// playwright's.
export function launchArgs(modelDir, baseModel) {
  return [
    // Playwright re-adds --use-angle outside the ignorable set, so it has to be overridden rather
    // than dropped; `default` hands the backend choice back to Chrome.
    '--use-angle=default',
    // Besides naming the borrowed weights, this waives the base-model version check Chrome applies
    // to a profile that has registered no component of its own.
...
// What the browser is launched with, out here for the same reason the switch list is: a guarantee
// nothing asserts is a guarantee until someone edits it.
...
async function openBrowser(profile, modelDir, baseModel, debug) {
  // The override switch loads the model, but the component installer decides whether anything is
  // MISSING, and a complete-looking profile starts no download. Best effort: the switch alone
  // suffices.
...
// Everything between a browser existing and a turn being possible, all of it inside the close: a
// launch that rejects is dropped from `sessions`, so anything left open is unreachable, holds a
// window, and keeps node alive.
...
// page.evaluate() has no timeout of its own, so the bound travels to the page with the request.
...
// Everything a close has to wait for: the turn in the page, and the launch a turn is still waiting
// on. One browser and one tab serve a whole row, so closing on the first caller to finish takes the
// tab from under the rest.
const turns = new Set()
...
// Reused across turns: no turn leaves state behind on the browser side, since each creates and
// destroys its own LanguageModel session.
function ensureSession(baseModel, debug) {
  if (!sessions.has(baseModel)) {
    // A rejected promise left in the map would be handed to every later turn, failing the rest of
    // the run instantly.
...
export async function closeChrome() {
  clearTimeout(idleClose)
  // Close when idle, not on demand: a caller finishing its turn while another is still mid-flight
  // would otherwise close that one's browser. A turn started during the drain keeps it going, which
  // is the same promise.
  while (turns.size > 0) await Promise.allSettled([...turns])
  const open = [...sessions.values()]
  sessions.clear()
  const shut = async (pending) => {
    // A launch that rejected already surfaced to its caller; raising it again here would go
    // unhandled.
...
// Chrome greets every page that touches the Prompt API with a feedback banner. Filtered by content
...
export async function turnInPage(req) {
  ...
  try {
    ...
    // Chrome's message for the interesting failure ends "Please check the result of availability()
  ...
  try {
    ...
    // The prompt itself is INPUT, and the post-prompt delta covers prompt, constraint context and
    // generated text together, so measuring it separately is what keeps it out of the completion
    // count.
    let promptTokens = 0
    ...
    return {
      ...
      // "Slow" here has two causes: create() pays to load gigabytes into the GPU the first time,
      // prompt() is the generation. One is amortised across a run, the other is not.
