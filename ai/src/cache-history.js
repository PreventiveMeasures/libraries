...
// string" is a problem of its own, with a recovery policy of its own.

// V8 caps string length at ~2^29 bytes and JSON.stringify throws this exact RangeError when a value
// serialises past it. Lets serializeHistory tell an oversized-history overflow (recoverable by
// dropping data) apart from a genuine bug (circular structure, BigInt) that should surface.
...
// RangeError is recovered; any other error surfaces.
...
// Null the `request` field on every entry except the first, returning a new array. Only
// entry[0].request is ever read back (cache-key recovery); the resume path reads none. Entries are
// shallow-copied (the original `request` objects, shared by reference with the live conversation,
// are never mutated) and the kept first entry passes through by reference.
...
function stripSignatures(value) {
  // `this` in a JSON replacer is the object holding `key`, so drop `signature` only on thinking
  // blocks and leave any unrelated field of the same name (e.g. a tool-call arg) untouched.
  // JSON.parse rebuilds a fresh tree, so no shared block is mutated.
