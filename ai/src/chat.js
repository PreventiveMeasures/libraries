...
// One conversation with a model, end to end: the first message, a turn per
// request, the caller's tool calls handed back and their results threaded
// into the next turn, and what the whole thing cost. What to ask and how to
// read the answer belong to the caller — nothing here inspects the text.
...
// Run one conversation to completion and return `{ text, error, usage,
// history }`. With no `tools` it is a single request; with them, the loop
// keeps going as long as the model keeps calling them.
//
// `maxToolTurns` caps the assistant↔tool round-trips: each turn is one
// API request whose response can include zero or more tool calls (the
// provider may batch). It is NOT a hard ceiling on total tool calls —
// the model can fan out N calls in a single turn — so callers that need
...
export async function ask({ model, maxTokens, systemPrompt, userContent, think = false, effort, tools, handleToolCall, maxToolTurns = DEFAULT_MAX_TOOL_TURNS, partial, onStart, debug, debugRequests, label, taskBudget = 'never', ...rest }) {
  ...
  const history = []
  const texts = []
  // One provider and one model for the whole conversation, so the wire-format
  // stamp every entry carries is resolved once.
  ...
  // recovers an oversized history's JSON.stringify overflow internally.
  ...
  if (previous) {
    ...
    // round-trip (and the cost / nondeterministic re-roll that would
    // come with it) when the partial got persisted but the final
    // setCache write didn't make it (process killed mid-finish).
  ...
  for (let turn = history.length; turn < maxToolTurns; turn++) {
    // Snapshot the pre-turn messages array before buildRequestBody —
    // appendToolResults mutates it in place, so a post-hoc capture
    // would leak the next turn's state into this entry.
    const preMessages = [...messages]
    // Head turn only: hold siblings sharing this prefix until the first one
    // replies, so they read the cache entry it writes instead of racing to
    // write their own. Released the moment the reply lands, not held for the
    // rest of the chain — later turns of this conversation are sequential and
    // extend a prefix nobody else shares.
    ...
    // task-budget=error path: the failed attempt's tokens still got paid
    // for, so account for both responses in the running total.
...
// `tool_use` blocks, OpenAI's tool_call arrays, etc.) so replaying
// an Anthropic-written partial under OpenRouter would feed the wrong
// shape to the next request. Per-entry stamps make the mismatch
// catchable. Legacy partials predating the stamp have
// `provider === undefined` and will fail the equality check when a
// provider is configured, which is the safe default.
//
// The last entry must additionally be a clean continuation point:
// either a terminal turn (no tool calls) or a tool-round with
// matching results, and it must NOT be flagged with an `error`.
// chat stamps `error` on entries persisted because of an
// API error or malformed tool args; resuming back into that state
// would just re-surface the same failure (or, for the short-circuit
// path, silently return empty text and mask the error).
...
// Per-attempt cost log, for every caller that drives a conversation.
// OpenRouter ships its own per-request cost number; everyone else relies on
// the local price table. `?? 0` is the unknown-model fallback so logging
// doesn't crash when calculateCost returns null.
//
// `pass` (optional) names the caller in the line — `[debug] Post-process
