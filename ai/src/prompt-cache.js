...
//   - the system prompt, the one thing stable for a whole run. On Anthropic it takes the 1h TTL
//     that top-level caching has no way to express, worth the 2.0x write premium; on gpt-5.6, whose
//     reads anchor to breakpoints, it is the only thing that makes the run-wide prefix reusable at
//     all;
//   - the isolate-mode prefix, where many per-export variants share a preamble and differ only in
//     the tail. The breakpoint has to sit at the end of the SHARED part; left to the end of the
//     whole message, every variant would write its own entry and none would ever be read.
// The conversation tail is not explicit: `cache_control` at the top level of the request
// auto-places on the last cacheable block and advances as the thread grows, which is exactly the
// bookkeeping we would otherwise hand-roll.

export const isAnthropicRoute = (model) => model.startsWith('anthropic/')

// The same question for OpenAI, which the gateway asks to pick the Responses route, on the same
// namespace-names-the-provider rule.
export const isOpenAIRoute = (model) => model.startsWith('openai/')

// The two routes whose provider reads Anthropic's `cache_control` marker out of the request body.
// Everywhere else it would be noise, so those routes send plain strings and let the provider cache
// on its own.
//
...
// than riding on the translation.
//
// An unlisted namespace gets no marker: the marker-free form is the one every gateway forwards
// unchanged. A provider that needs an explicit breakpoint is an edit here; a model-gated one, like
// gpt-5.6's, is a row in the registry.
export const readsExplicitBreakpoint = (model) => isAnthropicRoute(model) || model.startsWith('qwen/')

// The marker that ends the system prompt, in whichever dialect the model reads. Distinct from
// prefixMarker below only in its TTL: the system prompt is the one thing stable for an entire run,
// so Anthropic gets the 1h form and its 2.0x write premium rather than the 5m default.
//
// gpt-5.6 needs this as much as Anthropic does. Its implicit mode places a breakpoint on the LATEST
// message, so each file's request writes an entry ending in that file's own content and the next
...
// most.
// The one definition of the Anthropic system-prompt TTL, shared with the direct adapter so the two
...
// The whole user message as one string, which is what the routes that read no marker send and what
// the cache key is hashed over. `''` is the only join that can be used: cache-scan.js reads a
// stored request back into its key by flattening block content the same way, so any separator would
// put a re-scanned entry under a key its own request no longer produces.
...
// per-variant tail.
//
// Only ever one marker, however long the list. Blocks ahead of the shared part's end stay separate
// blocks and go out unmarked, because a second breakpoint would buy a second entry for a prefix the
// first already covers, at a write premium each. Where the boundary goes is the one thing this
// cannot guess, and the caller has already answered it by choosing which block to put last.
//
// Routes that read no marker concatenate instead. An unmarked multi-block user message buys
// nothing, since automatic caching matches the token prefix and a block boundary is not a cache
// boundary anywhere.
//
// gpt-5.6 is the case that makes this matter beyond Anthropic: it anchors cache reads to
// breakpoints instead of the 128-token strides earlier OpenAI models use, so without a marker at
// the end of the shared part every variant writes its own entry and none is ever read.
//
// A user message is documented placement for the marker on every route that has one; the growing
// conversation tail is not marked here at all, since assistant and tool messages are not, and the
// request-level field below covers it.
//
...
// exploring kind almost nothing: its head is a single sentence, so the entry it forgoes on turn 0
// is ~20 tokens.
