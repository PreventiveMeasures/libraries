...
// asked for. providers.js is the adapter; this is the one thing the adapter cannot answer on its
// own.
...
// to list its models is treated as having none.
const PROBE_TIMEOUT_MS = 2000

// One probe per origin, for the life of the process. A model pulled mid-run is not noticed, which
// costs a substitution rather than a turn.
...
async function probe(origin) {
  try {
    ...
    // Never fatal. The tag asked for is still a real tag, and the request about to be made reports
    // a down server better than a probe can.
    return new Set()
  }
}
...
// The tag to actually post. Separated from the probe so the rule is testable without a server: an
// alternative is taken only when it is installed, and the tag asked for is always a valid answer.
export function preferredTag(tag, installed) {
  // The row's own tag is what gets posted when nothing better is there; the rest are the same build
  // under another name, taken when the server has one.
