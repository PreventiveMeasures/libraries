...
describe('what it refuses about an entry', () => {
  const refused = [
    ...
    [{ name: 'a', uname: 'a\nb' }, /uname "a\\nb" holds a control or formatting character/u],
    ...
    [{ name: 'a', uname: 'u\uD800' }, /uname is not well-formed Unicode/u],
    [{ name: 'a', gname: '\uDFFF' }, /gname is not well-formed Unicode/u],
  ]
...
describe('what it refuses about names', () => {
  const refused = [
    ...
    [[{ name: 'a/b' }, { name: 'a' }], /"a" is already a directory, so it cannot be a file/u],
